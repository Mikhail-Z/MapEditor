...
class Writer:
    ...
    def __zbox(self, s):
        ...
        zbox = [min(z), max(z)]
        # update global
        self._zbox = [min(zbox[0], self._zbox[0]), min(zbox[1], self._zbox[1]), max(zbox[2], self._zbox[2]),
                      max(zbox[3], self._zbox[3])]
        return zbox
    ...
    def __mbox(self, shapes):
        ...
        mbox = [min(m), max(m)]
        # update global
        self._mbox = [min(mbox[0], self._mbox[0]), min(mbox[1], self._mbox[1]), max(mbox[2], self._mbox[2]),
                      max(mbox[3], self._mbox[3])]
        return mbox
